...
=====================
Generates Engine_ROM_Report_v4.pdf — cumulative document covering all 4 phases:
...
OUT   = os.path.join(RDIR, 'Engine_ROM_Report_v4.pdf')
...
# ── Phase 4 hardcoded scenario data ───────────────────────────────────────────
cl_scenarios = [
    {'name': 'S1_Rich_SA7',       'label': 'S1: Rich multi-step, SA=7°',       'rmse_tq': 3.6167, 'r2_tq': 0.99352, 'maxe_tq': 22.43, 'rmse_rpm': 291.63, 'r2_spd': 0.98878},
    {'name': 'S2_Rich_SA15',      'label': 'S2: Rich multi-step, SA=15°',      'rmse_tq': 2.3514, 'r2_tq': 0.99754, 'maxe_tq': 11.34, 'rmse_rpm': 160.97, 'r2_spd': 0.99657},
    {'name': 'S3_Rich_SA27',      'label': 'S3: Rich multi-step, SA=27°',      'rmse_tq': 2.2342, 'r2_tq': 0.99790, 'maxe_tq':  9.56, 'rmse_rpm': 139.90, 'r2_spd': 0.99747},
    {'name': 'S4_Throttle_Step',  'label': 'S4: Throttle step 5→50°, SA=15°', 'rmse_tq': 2.1753, 'r2_tq': 0.99477, 'maxe_tq': 10.19, 'rmse_rpm': 179.88, 'r2_spd': 0.99871},
    {'name': 'S5_SA_Step',        'label': 'S5: SA step 7→27°, Throttle=30°', 'rmse_tq': 0.8506, 'r2_tq': 0.99360, 'maxe_tq': 17.40, 'rmse_rpm':  29.90, 'r2_spd': 0.99938},
]
avg_rmse_tq  = 2.2457
avg_r2_tq    = 0.9955
avg_rmse_rpm = 160.45
...
class NumberedCanvas(canvas.Canvas):
    ...
    def _draw_page_number(self, total):
        ...
        self.drawString(L_MARGIN, PAGE_H - 1.5*cm,
                        'Engine ROM Report v4 – Closed-Loop Simulink Validation')
...
story.append(Paragraph('Version 4 – Closed-Loop Simulink Validation',
...
# Phase 4 highlights
story.append(Paragraph('Phase 4 Key Findings', H2))
highlights_v4 = [
    ...
    '— direct ROM output vs HiFi Combustion ground truth across all scenarios.',
    f'<b>Avg Closed-loop Speed RMSE = {avg_rmse_rpm:.1f} rpm, R²(speed) = {avg_r2_spd:.4f}</b> '
    '— downstream Vehicle Dynamics faithfully reproduce the HiFi engine speed trajectory.',
...
toc_items = [
    ...
    ('13', 'Phase 4 – Closed-Loop Simulink Validation'),
    ('14', 'Conclusions & Recommendations (v4)'),
...
para(
    ...
    'a Simulink physics model (<i>enginespeed.slx</i>). Four phases of work '
    'progressively reduce the memory footprint, improve the accuracy-vs-size '
    'trade-off, and culminate in a full closed-loop Simulink validation of the '
    'recommended ROM.'
)
para(
    ...
    '<b>Phase 4</b> (this document) closes the loop: the QAT LSTM-32 is '
...
para(
    ...
    'is recommended. '
    '<b>Phase 4 confirms</b> that the QAT ROM sustains R²(torque) = 0.9955 and '
    'R²(speed) = 0.9962 in closed-loop Simulink operation across all five '
    'transient scenarios.'
)
story.append(PageBreak())
...
val_models = [
    ...
    ['enginespeed_qat_sfun.slx',
     'QAT ROM S-Function replaces Combustion subsystem; '
     'same From Workspace inputs as HiFi model; closed-loop operation',
...
# ─────────────────────────────────────────────────────────────────────────────
# 13.6 Integration Summary
# ─────────────────────────────────────────────────────────────────────────────
subsection(13, 6, 'Integration Summary')
...
# ═════════════════════════════════════════════════════════════════════════════
section(14, 'Conclusions & Recommendations (v4)')
...
ph4_findings = [
    ...
    'closed-loop S-Function deployment (Phase 4, avg RMSE = 2.25 N·m on '
    'harder, longer transient scenarios).',
...
story.append(Paragraph(
    'Table 14.1 – Updated deployment guide including Phase 4 closed-loop results. '
...
next_steps_v4 = [
    ...
    'Closed-loop validation of Delta composite and LSTM-16 Q16 to extend '
    'the closed-loop Pareto frontier.',
    'True fixed-point implementation with LUT sigmoid/tanh for Cortex-M0+ '
    '(no FPU) targets.',
    'Mixed-precision QAT: int8 for gates, int16 for cell/hidden states.',
    'Online adaptation: periodically update polynomial baseline coefficients '
